(function ($, window, document, undefined) { //用一个自调用匿名函数把插架代码包裹起来，防止代码污染
	$.fn.mySelect = function (options) {
		...
		html += '<div class="select-picker-search-checked">请输入商品ID或商品名称</div>';
		html += '</div>';
		...
		$this.on('click', ".select-picker-options-list-item", function () {
			...
			if (checkedArr.length > 0) {
				...
				$this.find('.select-picker-search-checked').text('请输入商品ID或商品名称').css('color', '#757575');
			}
		})
		// 前端实现下拉搜索 
	...
	// 	$("#btn1").click(function () {
	// 		var selecttext = $(".select-picker-search-checked").text();
	// 		alert(selecttext)
	// 	});
	// });
